...
class userManager extends User {
    ...
    static async getUserByUsernameLogin(username, password) {
        ...
        try {
            ...
            data = await myClient.query("SELECT * FROM users where username = $1", [username]);
            console.log(data);
        } catch (err) {
        ...
        return (data.rowCount !== 0 ? data.rows[0].password === codedpassword ? data.rows[0] : console.log('password does not match') : null);
    }
